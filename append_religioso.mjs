...
const jsonPath = 'C:\\Users\\SEEMG\\Downloads\\deepseek_json_20260326_3a4b3b.json';
...
try {
    ...
    if (formattedQuestions.length === 0) {
        console.log("Nenhuma questão formatada.");
        process.exit(1);
    }
    ...
    content = content.replace(/\s*\];\s*$/, ',\n\n  // --- ENSINO RELIGIOSO LOTE 2 ---\n' + questionsStr + '\n];\n');
    
    fs.writeFileSync(questionsPath, content, 'utf-8');

    console.log(`\nSucesso! Adicionadas ${formattedQuestions.length} questões de Ensino Religioso.`);
