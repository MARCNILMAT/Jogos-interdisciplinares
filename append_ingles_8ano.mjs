...
const jsonFiles = [
    'C:\\Users\\SEEMG\\Downloads\\ingles_8ano_pt1.json',
    'C:\\Users\\SEEMG\\Downloads\\ingles_8ano_pt2.json'
];
...
try {
    ...
    for (const file of jsonFiles) {
        ...
        const gradeData = jsonData.series["8o_ano"];
    ...
    console.log(`\nSucesso! Adicionadas ${formattedQuestions.length} questões de Inglês (8º Ano).`);
    console.log(`Novo último ID: ${currentId - 1}`);

} catch (err) {
    fs.writeFileSync('error_ingles_8ano.log', err.stack || err.message);
    console.error(`Erro ao processar as questões. Veja error_ingles_8ano.log.`);
    process.exit(1);
}
