...
function getNextId() {
    ...
    if (!matches) return 3456;
    const ids = matches.map(m => parseInt(m.match(/\d+/)[0]));
    return Math.max(...ids) + 1;
}

const startId = getNextId();
console.log(`Lote Massive 71 (Artes, Inglês, Ed. Física e Religioso) a partir do ID: ${startId}`);
...
const batch = [
  ...
  { grade: "9º", discipline: "Ensino Religioso", type: "multiple", text: "O diálogo entre religiões em busca de união é o:", options: ["Ecumenismo", "Dialética", "Conflito", "Monólogo"], answer: "Ecumenismo", explanation: "Busca a cooperação inter-religiosa.", skill: "Tolerância" },
...
console.log(`Sucesso! Lote 71 de ${batch.length} questões multidisciplinares adicionado.`);
