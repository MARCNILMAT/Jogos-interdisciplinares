...
const jsonFiles = [
    'C:\\Users\\SEEMG\\Downloads\\ingles_9ano_pt1.json',
    'C:\\Users\\SEEMG\\Downloads\\ingles_9ano_pt2.json'
];
...
try {
    ...
    for (const file of jsonFiles) {
        ...
        const gradeData = jsonData.series["9o_ano"];
    ...
    content = content.replace(/\s*\];\s*$/, ',\n\n  // --- INGLÊS LOTE 4 (9º ANO) ---\n' + questionsStr + '\n];\n');
    
    fs.writeFileSync(questionsPath, content, 'utf-8');

    console.log(`\nSucesso! Adicionadas ${formattedQuestions.length} questões de Inglês (9º Ano).`);
    console.log(`Novo último ID: ${currentId - 1}. META DE 300 QUESTÕES ALCANÇADA!`);

} catch (err) {
    fs.writeFileSync('error_ingles_9ano.log', err.stack || err.message);
    console.error(`Erro ao processar as questões. Veja error_ingles_9ano.log.`);
    process.exit(1);
}
