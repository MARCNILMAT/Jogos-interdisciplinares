...
function getNextId() {
    ...
    if (!matches) return 3094;
    const ids = matches.map(m => parseInt(m.match(/\d+/)[0]));
    return Math.max(...ids) + 1;
}

const startId = getNextId();
console.log(`Lote Massive 52 (Inglês e Português) a partir do ID: ${startId}`);
...
const batch = [
  ...
  { grade: "6º", discipline: "Português", type: "multiple", text: "A palavra 'Pincel' é acentuada?", options: ["Sim", "Não", "Só no plural", "Só se for nome próprio"], answer: "Não", explanation: "Oxítonas terminadas em -el não levam acento.", skill: "Acentuação" },
  { grade: "8º", discipline: "Português", type: "multiple", text: "A figura de linguagem que consiste em atribuir características humanas a seres inanimados é a:", options: ["Metáfora", "Personificação (Prosopopeia)", "Hipérbole", "Ironia"], answer: "Personificação (Prosopopeia)", explanation: "Exemplo: 'O sol sorriu para mim'.", skill: "Figuras de Linguagem" }
];
...
console.log(`Sucesso! Lote 52 de ${batch.length} questões adicionado.`);
