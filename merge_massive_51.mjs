...
function getNextId() {
    ...
    if (!matches) return 3082;
    const ids = matches.map(m => parseInt(m.match(/\d+/)[0]));
    return Math.max(...ids) + 1;
}

const startId = getNextId();
console.log(`Lote Massive 51 (Multidiscip. - Volume Acelerado) a partir do ID: ${startId}`);
...
const batch = [
  ...
  { grade: "8º", discipline: "Ensino Religioso", type: "multiple", text: "A liberdade de crença no Brasil é um direito garantido pela:", options: ["Prefeitura", "Internet", "Constituição Federal", "Igreja Local"], answer: "Constituição Federal", explanation: "Nossa lei maior garante que todos podem ter sua religião.", skill: "Direitos" },
  { grade: "9º", discipline: "Ensino Religioso", type: "multiple", text: "O diálogo inter-religioso é importante para promover a:", options: ["Guerra entre religiões", "Cultura da Paz e Tolerância", "Divisão da sociedade", "Extinção das crenças"], answer: "Cultura da Paz e Tolerância", explanation: "O diálogo ajuda a superar preconceitos.", skill: "Tolerância" }
];
...
console.log(`Sucesso! Lote 51 de ${batch.length} questões multidisciplinares adicionado.`);
