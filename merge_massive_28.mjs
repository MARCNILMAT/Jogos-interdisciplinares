...
function getNextId() {
    ...
    if (!matches) return 2768;
    const ids = matches.map(m => parseInt(m.match(/\d+/)[0]));
    return Math.max(...ids) + 1;
}

const startId = getNextId();
console.log(`Lote Massive 28 (Multidisciplinar Premium) a partir do ID: ${startId}`);
...
console.log(`Sucesso! ${batch.length} questões do Lote 28 adicionadas.`);
