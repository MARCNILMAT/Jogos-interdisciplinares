...
function getNextId() {
    ...
    if (!matches) return 2834;
    const ids = matches.map(m => parseInt(m.match(/\d+/)[0]));
    return Math.max(...ids) + 1;
}

const startId = getNextId();
console.log(`Lote Massive 34 (Multidisciplinar Premium) a partir do ID: ${startId}`);
...
console.log(`Sucesso! ${batch.length} questões do Lote 34 adicionadas.`);
