...
const jsonFiles = [
    'C:\\Users\\SEEMG\\Downloads\\ingles_7ano_pt1.json',
    'C:\\Users\\SEEMG\\Downloads\\ingles_7ano_pt2.json'
];
...
try {
    ...
    for (const file of jsonFiles) {
        ...
        const gradeData = jsonData.series["7o_ano"];
    ...
    console.log(`\nSucesso! Adicionadas ${formattedQuestions.length} questões de Inglês (7º Ano).`);
    console.log(`Novo último ID: ${currentId - 1}`);

} catch (err) {
    fs.writeFileSync('error_ingles_7ano.log', err.stack || err.message);
    console.error(`Erro ao processar as questões. Veja error_ingles_7ano.log.`);
    process.exit(1);
}
